...
export default class Bit2C extends Component {
  constructor() {
    super();
    this.state = {
      bit2corderbookasks: null,
      bit2corderbookbids: null,
      bit2cticker: null
    };
  }
  ...
  componentDidMount() {
    axios
      .get(`http://localhost:3007/getbit2corderbook`)
      .then(res => {
        this.setState({
          bit2corderbookasks: res.data.asks,
          bit2corderbookbids: res.data.bids
        });
    ...
    axios
      .get(`http://localhost:3007/getbit2cticker`)
      .then(res => {
  ...
  render() {
    return <div className="Bit2C">I am Bit2C.</div>;
  }
}
