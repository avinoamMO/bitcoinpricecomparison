...
export function FeeSummaryBar({ exchanges }: FeeSummaryBarProps) {
  ...
  // Lowest taker fee
  const bestTaker = [...validExchanges].sort(
    (a, b) => a.fees.takerFee - b.fees.takerFee
  )[0];
  winners.push({
    category: "Lowest Taker Fee",
    winner: bestTaker.name,
    value: `${bestTaker.fees.takerFee}%`,
    description: "Best for market orders",
  });

  // Lowest maker fee
  const bestMaker = [...validExchanges].sort(
    (a, b) => a.fees.makerFee - b.fees.makerFee
  )[0];
  winners.push({
    category: "Lowest Maker Fee",
    winner: bestMaker.name,
    value: `${bestMaker.fees.makerFee}%`,
    description: "Best for limit orders",
  });

  // Tightest spread
  const withSpread = validExchanges.filter((e) => e.orderBook != null);
  if (withSpread.length > 0) {
    const bestSpread = [...withSpread].sort(
      (a, b) =>
        (a.orderBook?.spreadPercent ?? Infinity) -
        (b.orderBook?.spreadPercent ?? Infinity)
    )[0];
    winners.push({
      category: "Tightest Spread",
      winner: bestSpread.name,
      value: `${bestSpread.orderBook!.spreadPercent.toFixed(4)}%`,
      description: "Lowest bid-ask difference",
    });
  }
  ...
  // Best USD price
  const usdExchanges = validExchanges.filter(
    (e) =>
      e.price != null &&
  ...
  if (usdExchanges.length > 0) {
    const bestPrice = [...usdExchanges].sort(
      (a, b) => a.price! - b.price!
    )[0];
    winners.push({
  ...
  // Deepest liquidity
  if (withSpread.length > 0) {
    const deepest = [...withSpread].sort(
      (a, b) =>
        b.orderBook!.bidDepth +
        b.orderBook!.askDepth -
