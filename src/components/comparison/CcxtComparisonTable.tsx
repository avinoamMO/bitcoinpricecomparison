...
export function CcxtComparisonTable({
  ...
  // Sort by total effective cost (taker fee + spread)
  const sorted = [...exchanges].sort((a, b) => {
    const costA =
      a.fees.takerFee + (a.orderBook?.spreadPercent ?? 0);
    const costB =
      b.fees.takerFee + (b.orderBook?.spreadPercent ?? 0);
    return costA - costB;
  });
      ...
      <table className="w-full text-sm text-left border-collapse">
        ...
        <tbody>
          {sorted.map((ex, idx) => {
            ...
            const staticData = exchangeStaticData.find(
              (e) => e.id === ex.id
            );
            const affiliateUrl =
              staticData?.affiliateUrl || ex.feePageUrl;

            return (
              <tr
                key={ex.id}
                className={`border-b border-border/50 hover:bg-muted/50 transition-colors ${ex.israeliExchange ? "bg-crypto-blue/5" : ""}`}
              >
                ...
                <td className="py-3 px-3 text-center">
                  <span
                    className={`inline-block w-2.5 h-2.5 rounded-full ${ex.status === "ok" ? "bg-crypto-green" : "bg-crypto-red"}`}
                    title={
                      ex.status === "ok" ? "Live" : (ex.error ?? "Error")
                    }
                  />
                </td>
                <td className="py-3 px-3">
                  <a
                    ...
                    className="inline-flex items-center gap-1 px-3 py-1.5 rounded-lg text-xs font-medium bg-gold text-black hover:bg-gold-light transition-colors whitespace-nowrap"
                  >
                    Buy Here <ExternalLink className="h-3 w-3" />
                  </a>
                </td>
              </tr>
            );
